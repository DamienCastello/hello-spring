...
public class CityController {
    ...
    public ResponseEntity<?> getCity(@PathVariable Long id) throws FunctionalException{
            Optional<City> city = cities.stream().filter(el -> id.equals(el.getId())).findFirst();
            if(city.isEmpty()) throw new FunctionalException("Ville non trouvée");
            else return ResponseEntity.ok(city.get());
    }

    @GetMapping(params = {"name", "!population", "!minPop", "!maxPop"})
    public ResponseEntity<?> getCitiesStartWith(@RequestParam String name) throws FunctionalException{
        List<City> result = cities.stream().filter(el -> el.getName().toLowerCase().startsWith(name.toLowerCase())).toList();
        if(result.isEmpty()) throw new FunctionalException("Aucune ville trouvée pour avec les nom " + name);
    ...
    public ResponseEntity<?> getCitiesGreater(@RequestParam int population) throws FunctionalException{
        List<City> result = cities.stream().filter(el -> el.getPopulation() > population).toList();
        if(result.isEmpty()) throw new FunctionalException("Ville non trouvée avec une population supérieure à " + population);
        else return ResponseEntity.ok(result);
    }

    @GetMapping(params = {"minPop", "maxPop", "!population", "!name"})
    public ResponseEntity<?> getCitiesBetween(@RequestParam int minPop, @RequestParam int maxPop) throws FunctionalException{
        List<City> result = cities.stream().filter(el -> el.getPopulation() > minPop && el.getPopulation() < maxPop).toList();
        if(result.isEmpty()) throw new FunctionalException("Ville non trouvée entre " + minPop + " et " + maxPop);
        else return ResponseEntity.ok(result);
    }
    ...
    public ResponseEntity<String> createCity(@RequestBody City city) throws FunctionalException{
        ...
        if(exists) throw new FunctionalException("La ville existe déjà");
        if(city.getName().length() < 3) throw new FunctionalException("Le nom de la ville doit avoir au moins 3 caractère");
        if(city.getPopulation() < 10) throw new FunctionalException("La population minimale est 10");
    ...
    public ResponseEntity<String> deleteCity(@PathVariable Long id) throws FunctionalException{
        Optional<City> existingCity = cities.stream().filter(el -> id == el.getId()).findFirst();

        if(existingCity.isPresent()) {
            cities.remove(existingCity.get());
            return ResponseEntity.ok("La ville a été supprimée avec succès !");
        }
        else throw new FunctionalException("Ville non trouvée");
    }
}
